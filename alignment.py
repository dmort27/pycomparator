...
def normalize_segment(seg: str) -> str:
    """Normalize a segment to its canonical equivalent for alignment."""
    return EQUIVALENT_SEGMENTS.get(seg, seg)
...
def phoneme_distance(seg1: str, seg2: str) -> float:
    ...
    Equivalent segments (per EQUIVALENT_SEGMENTS) are treated as identical.
    ...
    """
    # Normalize segments to canonical equivalents before comparison
    seg1 = normalize_segment(seg1)
    seg2 = normalize_segment(seg2)
...
def pairwise_align(
    seq1: list[str], seq2: list[str], gap_cost: float = DEFAULT_GAP_COST
...
def star_align(
    sequences: list[list[str]], center_idx: int, gap_cost: float = DEFAULT_GAP_COST
) -> list[list[str]]:
    """
    ...
    for seq_idx, (aligned_center, aligned_seq) in enumerate(pairwise_alignments):
        ...
        for master_pos in range(master_len):
            ...
            gaps_before = sum(
                max_gaps_at_pos.get(p, 0)
                for p in range(orig_center_pos + 1)
                if p in max_gaps_at_pos and p <= orig_center_pos
            )
            expected_master_pos = orig_center_pos + sum(
                max_gaps_at_pos.get(p, 0) for p in range(orig_center_pos)
            )
    ...
    for seq_idx, (aligned_center, aligned_seq) in enumerate(alignments):
        ...
        for master_idx in range(len(master_center)):
            if master_center[master_idx] == "":
                ...
                while (
                    align_idx < len(aligned_center) and aligned_center[align_idx] == ""
                ):
